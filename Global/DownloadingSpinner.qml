...
// Usage: DownloadingSpinner { gameData: modelData; targetImage: boxFront; sourceBinding: function() { return boxArt(gameData) } }
    ...
    Connections {
        ...
        function onGameArtworkUpdated(updatedGameId) {
            if (updatedGameId === root.gameId && root.targetImage) {
                // Disable cache to force reload from disk
                root.targetImage.cache = false
                root.targetImage.source = ""

                // Use callLater to ensure assets are updated before re-binding
